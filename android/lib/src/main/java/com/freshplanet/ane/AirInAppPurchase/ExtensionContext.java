...
public class ExtensionContext extends FREContext {
        ...
        public FREObject call(final FREContext ctx, FREObject[] args) {
            ...
            if (purchaseId == null)
                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    @Override
                    public void run() {
                        _billingManager.purchaseProduct(ctx.getActivity(), purchaseId, null, -1, null, BillingClient.SkuType.INAPP, _purchaseFinishedListener);
                    }
                });
    ...
    private FREFunction makeSubscription = new BaseFunction() {
        ...
        public FREObject call(final FREContext ctx, FREObject[] args) {
            ...
            final String oldSubscriptionPurchaseToken = getStringFromFREObject(args[3]);
            ...
            if (purchaseId == null)
                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    @Override
                    public void run() {
                        _billingManager.purchaseProduct(ctx.getActivity(), purchaseId, oldPurchaseId, prorationMode, oldSubscriptionPurchaseToken, BillingClient.SkuType.SUBS, _purchaseFinishedListener);
                    }
                });
